        ...
        compiler.hooks.emit.tapAsync(VcdPluginVersion.name,
            (compilation, callback) => {
                if (!compilation.compiler.isChild()) {
                    const pomJson = xmlParser.parse(fs.readFileSync("pom.xml").toString().replace(/\r|\n/g, ""));
                    ...
                    manifestContent = manifestContent.replace("NAME_PLACEHOLDER",
                        `${pomJson.project.groupId}.${pomJson.project.artifactId}`);
                    manifestContent = manifestContent.replace("URN_PLACEHOLDER",
